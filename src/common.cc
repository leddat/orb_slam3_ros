...
ros::Publisher pose_pub, odom_pub, kf_markers_pub, odom_orb_pub;
...
void setup_publishers(ros::NodeHandle &node_handler, image_transport::ImageTransport &image_transport, std::string node_name, Eigen::Vector3d rpy_rad)
{
    ...
    odom_orb_pub = node_handler.advertise<nav_msgs::Odometry>(node_name + "/orb_odom", 1);
...
void publish_topics(ros::Time msg_time, Eigen::Vector3f Wbb)
{
    ...
    // publish_tf_to_base_transform(Twc, world_frame_id, "base_footprint", msg_time);
...
void publish_orb_odom(Sophus::SE3f Twb_SE3f, ros::Time msg_time)
{
    ...
    odom_msg.pose.pose.position.x = Twb_SE3f.translation().z();
    odom_msg.pose.pose.position.y = -Twb_SE3f.translation().x();
    odom_msg.pose.pose.position.z = 0.0;

    tf::Quaternion q(Twb_SE3f.unit_quaternion().coeffs().x(), Twb_SE3f.unit_quaternion().coeffs().y(), Twb_SE3f.unit_quaternion().coeffs().z(), Twb_SE3f.unit_quaternion().coeffs().w());
    q.normalize();
    tfScalar angle = q.getAngle();
    tf::Vector3 axis = q.getAxis();
    tf::Vector3 axis_new;
    if(axis.getY()>0)
    {
        axis_new = tf::Vector3(0, 0, -1);
    }
    else
    {
        axis_new = tf::Vector3(0, 0, 1);
    }
    tf::Quaternion q_new = tf::Quaternion(axis_new, angle);
    odom_msg.pose.pose.orientation.x = q_new.getX();
    odom_msg.pose.pose.orientation.y = q_new.getY();
    odom_msg.pose.pose.orientation.z = q_new.getZ();
    odom_msg.pose.pose.orientation.w = q_new.getW();
    odom_orb_pub.publish(odom_msg);
}
...
void publish_tf_to_base_transform(Sophus::SE3f T_SE3f, string frame_id, string child_frame_id, ros::Time msg_time)
{   
    tf::Transform tf_transform = SE3f_to_tfTransform(T_SE3f);
    tf::Transform tf_transform_base;
    tf_transform_base.setOrigin(tf::Vector3(tf_transform.getOrigin().getZ(), -tf_transform.getOrigin().getX(), 0));
    ...
    tf::Quaternion q_new = tf::Quaternion(axis_new, angle);
    tf_transform_base.setRotation(q_new);
    static tf::TransformBroadcaster tf_broadcaster;
    tf_broadcaster.sendTransform(tf::StampedTransform(tf_transform_base, msg_time, frame_id, child_frame_id));
}
